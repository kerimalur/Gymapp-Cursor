...
function getPlateColor(weight: number): string {
  switch (weight) {
    ...
    case 20: return 'bg-cyan-500';
    case 15: return 'bg-yellow-500';
    case 10: return 'bg-green-500';
    case 5: return 'bg-[hsl(225,14%,10%)] border-2 border-[hsl(225,10%,20%)]';
...
export function PlateCalculator({ targetWeight: initialWeight = 100, onClose }: PlateCalculatorProps) {
  ...
  return (
    <div className="bg-[hsl(225,14%,10%)] rounded-2xl shadow-xl p-6 max-w-2xl w-full">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-[hsl(var(--fg-primary))]">Scheiben-Rechner</h2>
          <p className="text-sm text-[hsl(var(--fg-muted))]">Berechne welche Scheiben du auflegen musst</p>
        </div>
        {onClose && (
          <button
            onClick={onClose}
            className="btn-icon hover:bg-[hsl(225,12%,18%)]"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <div>
          <label className="block text-sm font-medium text-[hsl(var(--fg-secondary))] mb-2">
            Zielgewicht (kg)
          </label>
          <input
            type="number"
            ...
            step={2.5}
            min={barbellWeight}
            className="w-full px-4 py-3 border-2 border-[hsl(225,10%,16%)] rounded-xl
                     focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20
                     bg-[hsl(225,12%,15%)] text-[hsl(var(--fg-primary))] outline-none transition-all text-lg font-semibold"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-[hsl(var(--fg-secondary))] mb-2">
      ...
      {/* Result Summary */}
      <div className="bg-gradient-to-br from-[hsl(225,12%,15%)] to-[hsl(225,12%,18%)] rounded-xl p-4 mb-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-[hsl(var(--fg-secondary))]">Tatsaechliches Gewicht</p>
            <p className="text-3xl font-bold text-[hsl(var(--fg-primary))]">{actualWeight} kg</p>
          </div>
          {Math.abs(difference) > 0.01 && (
            <div className="text-right">
              <p className="text-sm text-[hsl(var(--fg-secondary))]">Differenz</p>
      ...
      {plates.length > 0 ? (
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-[hsl(var(--fg-primary))]">Pro Seite auflegen:</h3>
          ...
          <div className="space-y-2">
            {plates.map((plate, index) => (
              <div
                key={index}
                className="flex items-center justify-between p-3 bg-[hsl(225,12%,13%)] rounded-lg"
              >
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 rounded-full ${plate.color} flex items-center justify-center shadow-md`}>
                    <span className={`text-sm font-bold ${plate.weight >= 5 ? 'text-white' : 'text-[hsl(var(--fg-secondary))]'}`}>
                      {plate.weight}
                    </span>
                  </div>
                  <div>
                    <p className="font-semibold text-[hsl(var(--fg-primary))]">{plate.weight} kg</p>
          ...
          <div className="mt-6">
            <h4 className="text-sm font-semibold text-[hsl(var(--fg-secondary))] mb-3">Visuelle Darstellung:</h4>
            <div className="flex items-center justify-center gap-1 p-4 bg-[hsl(225,12%,15%)] rounded-xl overflow-x-auto">
              {/* Left side plates */}
              ...
              <div className="flex flex-col items-center mx-2">
                ...
                <span className="text-xs font-semibold text-[hsl(var(--fg-secondary))] mt-1">
                  {barbellWeight} kg
                </span>
              </div>
          ...
          {/* Total Summary */}
            <div className="flex items-center justify-between p-4 bg-emerald-900/20 rounded-xl border-2 border-emerald-700">
            <div>
              <p className="text-sm text-emerald-300">Gesamt Scheiben (beide Seiten)</p>
              <p className="text-xl font-bold text-emerald-100">
                {plates.reduce((sum, p) => sum + p.count, 0) * 2} Scheiben
              </p>
            </div>
            <div className="text-right">
              <p className="text-sm text-emerald-300">Scheiben-Gewicht</p>
              <p className="text-xl font-bold text-emerald-100">
      ...
      <div className="mt-6 pt-6 border-t border-[hsl(225,10%,16%)]">
        <p className="text-sm font-medium text-[hsl(var(--fg-secondary))] mb-3">Schnellauswahl:</p>
